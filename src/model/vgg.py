...
class VGG_GAP_FC(nn.Module):

    def __init__(self, features, num_classes=1000, init_weights=True, student_dim=0, force_2FC=False):
        super(VGG_GAP_FC, self).__init__()
        self.features = features
...
def _vgg_gap_fc(cfg, batch_norm, num_classes, **kwargs):
    model = VGG_GAP_FC(make_layers(cfgs[cfg], batch_norm=batch_norm), num_classes, **kwargs)
    return model
