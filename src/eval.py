...
from .utils import AverageMeterSet, Prec1

def validate(eval_loader, model, epoch, writer, LOG, print_freq, type_string=''):
    ...
    if writer:
        writer.add_scalar("{}val/prec1".format(type_string), meters['top1'].avg, epoch)
...
def validate_coco(eval_loader, model, thre, epoch, writer, LOG, print_freq):
    start_time = time.time()
    meters = AverageMeterSet()
    ...
    LOG.info(' * CP {:.2f} CR {:.2f} CF1 {:.2f} OP {:.2f} OR {:.2f} OF1 {:.2f}'
          .format(CP, CR, CF1, OP, OR, OF1))

    mAP_score = mAP(targs, preds)
    LOG.info(" * TEST [{}] mAP: {}".format(epoch, mAP_score))
    if writer:
        writer.add_scalar("test/map", mAP_score, epoch)
    LOG.info("--- testing epoch in {} seconds ---".format(time.time() - start_time))
    return mAP_score


def validate_voc(eval_loader, model, thre, epoch, writer, LOG, print_freq):
    start_time = time.time()
    ...
    mAP = 100 * AP.mean()
    LOG.info(" * TEST [{}] mAP: {}".format(epoch, mAP))
    if writer:
...
def validate_voc_file(eval_loader, model, thre, epoch, writer, LOG, print_freq, results_dir):
    start_time = time.time()
    ...
    mAP = 100 * AP.mean()
    LOG.info(" * TEST [{}] VOC2012 mAP: {}".format(epoch, mAP))
    if writer:
        writer.add_scalar("test/map", mAP, epoch)
